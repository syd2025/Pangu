...
func (m *TokenModel) insert(token *Token) error {
	query := `
		INSERT INTO appuser.tokens (hash, user_id, expiry, scope)
		VALUES ($1, $2, $3, $4)`
	...
	tx := m.DB.WithContext(ctx).Exec(query, args...)
	return tx.Error
}

func (m TokenModel) DeleteAllForUser(scope string, userID int64) error {
	query := `
		DELETE FROM appuser.tokens
		WHERE scope = $1 AND user_id = $2`
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	tx := m.DB.WithContext(ctx).Exec(query, scope, userID)
	return tx.Error
}

func (m TokenModel) DeleteAllExpired() error {
	query := `
		DELETE FROM appuser.tokens
		WHERE expiry < $1`
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	...
	tx := m.DB.WithContext(ctx).Exec(query, time.Now())
	return tx.Error
}

func (m TokenModel) DeleteToken(tokenPlaintext string) error {
	...
	tx := m.DB.WithContext(ctx).Exec(query, tokenHash[:])
	return tx.Error
}
